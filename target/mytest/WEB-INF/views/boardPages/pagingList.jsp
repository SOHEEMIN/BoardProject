...
<div class="container mt-3"> <!--mt: 마진 (3만큼 간격을 떨어뜨린다)-->
...
<div class="container">
    <ul class="pagination justify-content-center">
        ...
        <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i" step="1">
            <%--forEach begin: 시작값 / end: 끝값 / var: 반복변수 / step:증감
                startPage는 앞 service에서 설정했듯이 1, 4, 7페이지로 고정
                endPage는 3, 6, 9, 12이나 max페이지가 마지막 페이지
                ex)for(int i=startPage; i<=endPage; i++) 과 같음--%>
            <c:choose>
                <c:when test="${i eq paging.page}">
                    <%--요청한 페이지에 있는 경우 현재페이지 번호는 텍스트만 보이게
                        eq = equals 라는 뜻--%>
